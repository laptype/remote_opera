...
def bbox_kpt2result(bboxes, labels, poses, shapes, trans, num_classes):
    ...
    if bboxes.shape[0] == 0:
        return [np.zeros((0, 5), dtype=np.float32) for i in range(num_classes)], \
            [np.zeros((0, poses.size(1)), dtype=np.float32)
                for i in range(num_classes)], \
            [np.zeros((0, shapes.size(1)), dtype=np.float32)
                        for i in range(num_classes)]
    else:
        ...
        return [bboxes[labels == i, :] for i in range(num_classes)], \
            [poses[labels == i, :] for i in range(num_classes)], \
            [shapes[labels == i, :] for i in range(num_classes)], \
            [trans[labels == i, :] for i in range(num_classes)]
            # [joints[labels == i, :, :] for i in range(num_classes)], \
            # [verts[labels == i, :, :] for i in range(num_classes)]
