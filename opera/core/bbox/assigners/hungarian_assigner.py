...
class MeshHungarianAssigner(BaseAssigner):
    ...
    def __init__(self,
                 ...
                 pose_cost=dict(type='MSECost', weight=1.0),
                 trans_cost=dict(type='MSECost', weight=1.0),
                 shape_cost=dict(type='MSECost', weight=1.0),
                 joints_cost=dict(type='KptL1Cost', weight=1.0),):
        self.cls_cost = build_match_cost(cls_cost)
        self.kpt_cost = build_match_cost(kpt_cost)
        self.pose_cost = build_match_cost(pose_cost)
        self.trans_cost = build_match_cost(trans_cost)
        self.shape_cost = build_match_cost(shape_cost)
        self.joints_cost = build_match_cost(joints_cost)
    ...
    def assign(self,
        ...
        #joints regression L1 cost
        joints_pred_tmp = joints_pred.clone().detach().reshape(
            joints_pred.shape[0], -1, 3)
        joints_cost = self.joints_cost(joints_pred_tmp, gt_keypoints_reshape,
                                 valid_kpt_flag)
        

        pose_cost = self.pose_cost(pose_pred, gt_poses)
        shape_cost = self.shape_cost(shape_pred, gt_shapes)
        trans_cost = self.trans_cost(trans_pred, gt_trans)

        cost = cls_cost + kpt_cost + pose_cost + shape_cost + joints_cost + trans_cost
        # lanbo shan
        # print('cls_cost', cls_cost)
        # print('kpt_cost', kpt_cost)
        # print('pose_cost', pose_cost)
        # print('shape_cost', shape_cost)
        # print('joints_cost', joints_cost)
