...
from mmcv.cnn import constant_init, xavier_init, kaiming_init
...
class PetrTransformerDecoder(TransformerLayerSequence):
    ...
    def forward(self,
        ...
        intermediate_reference_poses = []
        intermediate_reference_trans = []
        for lid, layer in enumerate(self.layers):
            ...
            if kpt_branches is not None:
                tmp_points = kpt_branches[lid](output)
                if reference_points.shape[-1] == self.num_keypoints * 3:
                    new_reference_points = tmp_points + reference_points
                else:
                    raise NotImplementedError
                reference_points = new_reference_points.detach()

            if pose_branches is not None:
                tmp_poses = pose_branches[lid](output)
                new_reference_poses = tmp_poses + reference_poses
                reference_poses = new_reference_poses.detach()

            if trans_branches is not None:
                tmp_trans = trans_branches[lid](output)
                new_reference_trans = tmp_trans + reference_trans
                reference_trans = new_reference_trans.detach()

            output = output.permute(1, 0, 2)
            if self.return_intermediate:
                intermediate.append(output)
                intermediate_reference_points.append(reference_points)
                intermediate_reference_poses.append(reference_poses)
                intermediate_reference_trans.append(reference_trans)

        if self.return_intermediate:
            return torch.stack(intermediate), torch.stack(
                intermediate_reference_points), torch.stack(intermediate_reference_poses), \
                torch.stack(intermediate_reference_trans)

        return output, reference_points, reference_poses, reference_trans
...
class PetrRefineTransformerDecoder(TransformerLayerSequence):
    ...
    def forward(self,
        ...
        intermediate_reference_trans = []
        for lid, layer in enumerate(self.layers):
            assert reference_poses.shape[-1] == 6
            output = layer(
                output,
                *args,
                **kwargs)
            ...
            if pose_branches is not None:
                # tmp = reg_branches[lid](output[:, 1:])
                tmp_pose = pose_branches[lid](output[:, 1:])
                # #test 
                assert reference_poses.shape[-1] == 6
                new_reference_poses = tmp_pose + reference_poses
                reference_poses = new_reference_poses.detach()

            if trans_branches is not None:
                # tmp = reg_branches[lid](output[:, 1:])
                tmp_trans = trans_branches[lid](output[:, 0])
                assert reference_trans.shape[-1] == 3
                new_reference_trans = tmp_trans + reference_trans
                reference_trans = new_reference_trans.detach()
            ...
            if self.return_intermediate:
                intermediate.append(output)
                intermediate_reference_points.append(reference_poses)
                intermediate_reference_trans.append(reference_trans)
        ...
        return output, reference_poses, reference_trans
...
class PETRTransformer(Transformer):
    ...
    def __init__(self,
        ...
        if self.generated_query:
            queries_generator = Query_Generator(self.embed_dims)
            self.get_queries = _get_clones(queries_generator, 22)
    ...
    def init_layers(self):
        ...
        if self.as_two_stage:
            ...
            if not self.generated_query:
                self.refine_query_embedding = nn.Embedding(22, self.embed_dims)
            self.refine_query_pos_embedding = nn.Embedding(23, self.embed_dims)
        else:
    ...
    def random_masking(self, x):
        ...
        x_tmp = torch.ones(N, L - len_keep, D)*0.01
        x_tmp = x_tmp.to(x.device)
        x = torch.cat((x_masked, x_tmp), dim=1)
    ...
    def forward(self,
        ...
        memory = self.encoder(
            ...
            key=None,
            value=None,
            query_pos=self.pos_embed.unsqueeze(1).to(feat_flatten.device),
            **kwargs)
        ...
        if self.as_two_stage:
            enc_outputs_class = cls_branches[self.decoder.num_layers](
                memory)
            enc_outputs_shape = shape_branches[self.decoder.num_layers](
                memory)
            
            enc_outputs_kpt_unact = \
                kpt_branches[self.decoder.num_layers](memory)
            enc_outputs_pose = \
                pose_branches[self.decoder.num_layers](memory)
            enc_outputs_trans = \
                trans_branches[self.decoder.num_layers](memory)
            ...
            init_reference_points_out = reference_points

            topk_poses = torch.gather(
                enc_outputs_pose, 1,
                topk_proposals.unsqueeze(-1).repeat(
                    1, 1, enc_outputs_pose.size(-1)))
            topk_poses = topk_poses.detach()

            reference_poses = topk_poses
            init_reference_pose_out = reference_poses

            topk_trans = torch.gather(
                enc_outputs_trans, 1,
                topk_proposals.unsqueeze(-1).repeat(
                    1, 1, enc_outputs_trans.size(-1)))
            topk_trans = topk_trans.detach()

            reference_trans = topk_trans
            init_reference_trans_out = reference_trans
        ...
        inter_states, inter_references_points, inter_references_poses, inter_references_trans = self.decoder(
            query=query,
            # key=None,
            key=memory,
            value=memory,
            ...
            pose_branches=pose_branches,
            trans_branches=trans_branches,
            **kwargs)

        inter_references_points_out = inter_references_points
        inter_references_poses_out = inter_references_poses
        inter_references_trans_out = inter_references_trans
        if self.as_two_stage:
            return inter_states, init_reference_points_out, init_reference_pose_out, init_reference_trans_out,\
                   inter_references_points_out, inter_references_poses_out, inter_references_trans_out, \
                    enc_outputs_class, enc_outputs_kpt_unact, enc_outputs_pose, enc_outputs_trans, enc_outputs_shape, memory
        return inter_states, init_reference_points_out, init_reference_pose_out, \
               inter_references_points_out, inter_references_poses_out, None, None, None, None, None
    ...
    def forward_refine(self,
        ...
        if self.generated_query:
            ...
            query = torch.stack(query_list, 1)
        else:
            query = self.refine_query_embedding.weight
            query = query.unsqueeze(0).expand(pos_num, -1, -1)
        query_pos = self.refine_query_pos_embedding.weight
        query_pos = query_pos.unsqueeze(0).expand(pos_num, -1, -1)

        reference_poses = reference_points_pose.reshape(
            pos_num,
        ...
        query = torch.cat((human_queries.unsqueeze(1), query), 1)
        ...
        init_reference_pose_out = reference_poses
        init_reference_trans_out = reference_trans
        return inter_states, init_reference_pose_out, inter_references_pose, \
            init_reference_trans_out, inter_references_trans
...
class Query_Generator(nn.Module):
    def __init__(self, embed_dim):
        ...
        self.linear_layer1 = nn.Linear(embed_dim, embed_dim)
        self.relu_layer1 = nn.LeakyReLU()

        self.linear_layer2 = nn.Linear(embed_dim, embed_dim)
        self.relu_layer2 = nn.LeakyReLU()

        self.linear_layer3 = nn.Linear(embed_dim, embed_dim)
        self.relu_layer3 = nn.LeakyReLU()
        # self.init_weights()

    def init_weights(self):
        kaiming_init(self.linear_layer1, nonlinearity='leaky_relu', distribution='uniform')
        kaiming_init(self.linear_layer2, nonlinearity='leaky_relu', distribution='uniform')
        kaiming_init(self.linear_layer3, nonlinearity='leaky_relu', distribution='uniform')

    def forward(self, x):
        x_0 = x
        x_0 = self.linear_layer1(x_0)
        x_0 = self.relu_layer1(x_0)

        x = x + x_0

        x_0 = x
        x_0 = self.linear_layer2(x_0)
        x_0 = self.relu_layer2(x_0)

        x = x + x_0

        x_0 = x
        x_0 = self.linear_layer3(x_0)
        x_0 = self.relu_layer3(x_0)
