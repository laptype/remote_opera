...
class PETR(DETR):
    ...
    def __init__(self, has_phase=True, amp_wdt=True, *args, **kwargs):
        ...
        if self.amp_wdt:
            ...
            amp_head = []
            amp_head.append(Linear(30, 512))
            amp_head.append(nn.ReLU())
            amp_head.append(Linear(512, 512))
            amp_head.append(nn.ReLU())
            self.amp_head = nn.Sequential(*amp_head)
            element_nums = 1
        ...
        head = []
        head.append(Linear(512*element_nums, 512))
        head.append(nn.ReLU())
    ...
    def forward_train(self,
        ...
        if self.amp_wdt:
            ...
            x = torch.cat((amp_Hdim, amp_Ldim), -1)
            pha_start = 60
        else:
            amp = img[..., :30]
            x = self.amp_head(amp)
        ...
        losses = self.bbox_head.forward_train(x, img_metas, gt_bboxes,
                                              ...
                                              gt_areas, cam_trans, gt_bboxes_ignore)
        return losses
    ...
    def simple_test(self, img, img_metas, rescale=False):
        ...
        """
        # torch.save(self.state_dict(), '/home/qianbo/git/opera-version1/results/test_val/save.pth')
        ...
        bbox_kpt_results = [
            bbox_kpt2result(det_bboxes, det_labels, det_poses, det_shapes, det_trans,
                            self.bbox_head.num_classes)
            for det_bboxes, det_labels, det_poses, det_shapes, det_trans in results_list
        ]
